jQuery(document).ready(function($){
	...
	$('#download').on('click', function(e){
		...
		$.post(ajaxurl, { action: 'update_ip2location_redirection_database', database: $('#database_name').val(), token: $('#token').val() }, function(response) {
			if (response == 'SUCCESS') {
	...
	function insert_rule(country_code, from, to, url_from, url_to, http_code){
		...
		var http_codes = {"301":"301 Permanently Redirect", "302":"302 Temporary Redirect"};
		var exclude = false;
		...
		var $from_list = $('<select name="from[]" data-placeholder="Choose Page..." class="chosen redirect-from">')
			.on('change', function(){
				if($(this).val() != 'url' && $(this).val() == $(this).parent().next().children().val()){
					alert('The value of [From] and [Destination] cannot be same.d');
					$(this).val('');
					$('.chosen').trigger('chosen:updated');
					return;
				}
		...
		var $destination_list = $('<select name="to[]" data-placeholder="Choose Destination..." class="chosen redirect-to">')
			.on('change', function(){
				if($(this).val() != 'url' && $(this).val() == $(this).parent().prev().children().val()){
		...
		var $rule = $('<tr>')
			...
			.append($('<td>').append($from_list).append('<div class="url-container" style="display:' + ((from == 'url') ? 'block' : 'none') + '"><input type="text" name="url_from[]" value="' + url_from + '" class="url regular-text" maxlength="255" /></div>'))
			.append($('<td>').append($destination_list).append('<div class="url-container" style="display:' + ((to == 'url') ? 'block' : 'none') + '"><input type="text" name="url_to[]" value="' + url_to + '" class="url regular-text" maxlength="255" /></div>'))
